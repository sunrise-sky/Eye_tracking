...
│       ├── pupil_gap.m1model    # 旧版瞳孔恢复模型
│       └── pupil_a1_opset11.m1model # 综合模式默认使用的新训练模型
├── cmake_config/
...
综合模式默认使用新训练的 `pupil_a1_opset11.m1model` 恢复低置信瞳孔结果。模型输入为 224x224 眼区图像，输出归一化瞳孔坐标。模型输出会再映射回原始图像坐标。
...
export PUPIL_GAP_MODEL=/app_demo/app_assets/models/pupil_a1_opset11.m1model
...
- `PUPIL_GAP_MODEL`：可选瞳孔模型路径；未设置时综合模式使用 `pupil_a1_opset11.m1model`。
...
- `task/tracking.cpp`：`PupilDetector::InitializeModel()` 加载配置的瞳孔模型并准备模型输入 tensor。
...
app_assets/models/pupil_a1_opset11.m1model
```
...
```text
/app_demo/app_assets/models/face_640x480.m1model
/app_demo/app_assets/models/pupil_a1_opset11.m1model
```
